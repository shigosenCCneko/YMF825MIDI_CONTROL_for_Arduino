...
extern uint8_t channel_noteno[16];
extern const PROGMEM uint8_t fnum_hi_tbl[128];
extern const PROGMEM uint8_t fnum_lo_tbl[128];
...
void init_midich(void) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    midi_ch[i].s_modulation_pitch = 13;
    midi_ch[i].s_modulation_depth = 2;
...
void note_on2(uint8_t midich, uint8_t voicech, uint8_t note_no, uint8_t velo, uint8_t voice_no) {
...
//    if_s_write(0x12, hi);
    ...
    if_chs_write(p->voice_ch,0x12,hi);
    if_s_write(0x13, lo);
//    sei();
    p = p->next;
  }
